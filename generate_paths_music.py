...
class IIPath:
        ...
        with open(outfile, 'w') as file:
            for i in all_item_ids:
                ...
                if i%100 ==0:
                    this_time = time.time() - start_time
                    print(f'processed item: {i}, time: {this_time}')
    def save_iuici(self, all_item_ids, start_time, outfile):
        limit = 1
        outfile = self.outputfolder+'iuici.paths'
        with open(outfile, 'w') as file:
                        ...
                        for c in c_list:
                            ...
                            for i3 in i3_list:
                                if(count>=limit):
                                    continue
                                if self.ifInIIpairs(i, i3):
                                    path = str(i) + ' ' + str(u1) + ' ' + str(i2) + ' ' + str(c) + ' ' + str(i3)
                                    path_id = str(i) + ',' + str(i3)
                                    write_content = path_id + '\t' + path + '\n'
    ...
    def save_iuuii(self, all_item_ids, start_time, outfile):
        limit = 1
        outfile = self.outputfolder+'iuuii.paths'
        with open(outfile, 'w') as file:
                        ...
                        for i2 in i2_list:
                            ...
                            for i3 in i3_list:
                                if(count>=limit):
                                    continue
                                if self.ifInIIpairs(i, i3):
                                    path = str(i) + ' ' + str(u1) + ' ' + str(u2) + ' ' + str(i2) + ' ' + str(i3)
                                    path_id = str(i) + ',' + str(i3)
                                    write_content = path_id + '\t' + path + '\n'
    ...
    def save_iiuui(self, all_item_ids, start_time, outfile):
        limit = 1
        outfile = self.outputfolder+'iiuui.paths'
        with open(outfile, 'w') as file:
                        ...
                        for u3 in u3_list:
                            ...
                            for i3 in i3_list:
                                if(count>=limit):
                                    continue
                                if self.ifInIIpairs(i, i3):
                                    path = str(i) + ' ' + str(i1) + ' ' + str(u2) + ' ' + str(u3) + ' ' + str(i3)
                                    path_id = str(i) + ',' + str(i3)
                                    write_content = path_id + '\t' + path + '\n'
    ...
    def metapath_based_randomwalk(self):
        ...
        for metapath in self.metapath_list:
            ...
            if metapath == 'iuici':
                self.save_iuici(all_item_ids, start_time, outfile)
            if metapath == 'iuuii':
                self.save_iuuii(all_item_ids, start_time, outfile)
            if metapath == 'iiuui':
                self.save_iiuui(all_item_ids, start_time, outfile)
...
def gen_instances(embeddings,refine_path,user_history_path,out_path):
    ...
    #                        'iuiui']
    ii_metapahts_list = ['icici','iciui','iuici','iuiui','iuuii','iiuui',
                         'iui','ici','iii']
    IIPath(ic_relation=ic_relation, ug_relation=ug_relation,ui_relation = ui_relation,
...
def count_nodes(time_count):
    ...
    all_nodes = user_num + item_num + category_num 
