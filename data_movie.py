...
def get_item_meta(metafile,outputoldfolder):
    movies = metafile +"movies.dat"
    users = metafile +"users.dat"
    item_category = []
    user_gender = []
    user_age = []
    user_occupation = []
    ...
    for line in lines:
        genres_list =[]
        movie_id, title, genres_string = line.strip().split('::')
        genres = genres_string.split('|')
        genres_list.extend(genres)
        for cate in genres_list:
            item_category.append(['i_'+movie_id, 'c_'+cate])
    with open(users, 'r', encoding='latin-1') as file_user:
        lines = file_user.readlines()
    for line in lines:
        user_id, gender, age,occupation,_ = line.strip().split('::')
        user_gender.append(['u_'+user_id, 'g_'+gender])
    ...
    user_gender_df = pd.DataFrame(user_gender)
...
def form_ids(oldrefinefolder,file_path,refine_path,little_path,index,item_category_df,user_gender_df,user_age_df,user_occupation_df):
    ...
    all_user_gender_df = user_gender_df
    ...
    # 全部节点数
    all_allnodes = list(all_user_set) + list(all_item_set) + list(all_category_set) + list(all_user_gender_set)+ list(all_user_age_set) + list(all_user_occupation_set)

    allnodes = list(user_set) + list(item_set) + list(category_set) + list(gender_set)+ list(age_set) + list(occupation_set)
    """
...
if __name__ == '__main__':
    ...
    for index in range(time_count):
        ...
        all_allnodes = form_ids(oldrefinefolder,file_path,refine_path,little_path,index,item_category_df,user_gender_df,user_age_df,user_occupation_df)
