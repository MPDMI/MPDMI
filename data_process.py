...
def process_initial_ratings(ratings_csv,output_ratings_csv,fit_item_csv):
...
def get_item_meta(metafile,outputoldfolder):
    ...
    with open(metafile, 'r') as f:
        ...
        lines = f.readlines()
        for line in lines:
            # json.loads() 函数用于将 JSON 格式的字符串转换为 Python 字典对象。
            ...
            if 'title' in line_json.keys():
                item_title.append([line_json['asin'], line_json['title']])

    item_category_df = pd.DataFrame(item_category)
    item_brand_df = pd.DataFrame(item_brand)
    item_item_df = pd.DataFrame(item_item)
    item_title_df = pd.DataFrame(item_title)
    item_category_df=item_category_df.drop_duplicates()
    item_brand_df = item_brand_df.drop_duplicates()
    item_item_df = item_item_df.drop_duplicates()
    item_title_df = item_title_df.drop_duplicates()

    ic_filename = outputoldfolder + 'item_category.csv'
    ib_filename = outputoldfolder + 'item_brand.csv'
    ii_filename = outputoldfolder + 'item_item.csv'
    it_filename = outputoldfolder + 'item_title.csv'
    ...
    item_title_df.to_csv(it_filename, header=None, index=None, sep=',')
    print(f'saved items meta to folder: {outputoldfolder}')

def form_ids(oldrefinefolder,file_path,refine_path,little_path,index,item_category_df,item_brand_df,item_item_df):
    ...
    # 筛选出所有项目的集合
    all_item_category_df = item_category_df[item_category_df[0].isin(list(all_item_set))]
    ...
    all_item_brand_df = item_brand_df[item_brand_df[0].isin(list(all_item_set))]
    item_brand = item_brand_df[item_brand_df[0].isin(list(item_set))]

    all_brand_set = all_item_brand_df[1].unique()
    brand_set = item_brand[1].unique()
    # print(item_brand_df)


    all_item_item_df = item_item_df[(item_item_df[0].isin(list(all_item_set))) & (item_item_df[1].isin(list(all_item_set)))]
    
    item_item = item_item_df[(item_item_df[0].isin(list(item_set))) & (item_item_df[1].isin(list(item_set)))]

    # print(item_item_df)
    # 完成了数据的筛选，使之只保存符合要求的物品
    all_refinefolder = oldrefinefolder
    ic_refine = all_refinefolder + 'item_category_refine.csv'
    ib_refine = all_refinefolder + 'item_brand_refine.csv'
    ii_refine = all_refinefolder + 'item_item_refine.csv'
        
    all_item_category_df.to_csv(ic_refine, header=None, index=None, sep=',')
    all_item_brand_df.to_csv(ib_refine, header=None, index=None, sep=',')
    all_item_item_df.to_csv(ii_refine, header=None, index=None, sep=',')
    ...
    item_brand.to_csv(ib_refine, header=None, index=None, sep=',')
    item_item.to_csv(ii_refine, header=None, index=None, sep=',')
    ...
    # 全部节点数
    all_allnodes = list(all_user_set) + list(all_item_set) + list(all_category_set) + list(all_brand_set)
    ...
    item_item_r = []
    user_item_r = []
    this_user_item_r =[]  # user_id, item_id, timestamp
    # 把各个属性代表的id连接起来
    ...
    for _, row in item_brand.iterrows():
        item_id = name2id[row[0]]
        brand_id = name2id[row[1]]
        item_brand_r.append([item_id, brand_id])
    item_brand_relation = pd.DataFrame(item_brand_r)
    item_brand_relation.to_csv(ib_relation, header=None, index=None, sep=',')

    for _, row in item_item.iterrows():
        item1_id = name2id[row[0]]
        item2_id = name2id[row[1]]
        item_item_r.append([item1_id, item2_id])
    item_item_relation = pd.DataFrame(item_item_r)
...
if __name__ == '__main__':
    ...
    metafile = "./"+str(model_name)+"/data/"+str(data_name)+"/old/meta_"+str(data_name)+".json"    
    # 原始的交互记录
    ...
    process_initial_ratings(ratings_csv,output_ratings_csv,fit_item_csv)
    ...
    for index in range(time_count):
        ...
        all_allnodes = form_ids(oldrefinefolder,file_path,refine_path,little_path,index,item_category_df,item_brand_df,item_item_df)
